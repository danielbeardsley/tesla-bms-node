   ...
   bms: z.object({
      ...
      chargingStrategy: z.object({
         ...
         latterby: z.optional(z.object({
            ...
            // Overrides for the SOC window used when high-capacity mode is
            // active (see storage.highCapacityModeValidUntil). Any key
            // omitted falls back to the corresponding normal value, so this
            // can be a full alternate window or a partial widening.
            highCapacity: z.optional(z.object({
               stopChargeAtPct: z.number().min(0).max(100).optional(),
               resumeChargeAtPct: z.number().min(0).max(100).optional(),
               stopDischargeAtPct: z.number().min(0).max(100).optional(),
               resumeDischargeAtPct: z.number().min(0).max(100).optional(),
            })),
         })),
      }),
   }),
